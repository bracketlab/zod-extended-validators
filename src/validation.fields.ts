...
export const requiredPositiveNumberField = (
  ...
  return z
    .union([
      requiredNumberField().min(0, {
        message:
          customError ?? EValidationErrors.ERROR_MUST_BE_POSITIVE_DECIMAL_NUMBER
      }),
      requiredTextField().refine(
        val => REGEX_POSITIVE_DECIMAL_NUMBER.test(val),
        {
          message:
            customError ??
            EValidationErrors.ERROR_MUST_BE_POSITIVE_DECIMAL_NUMBER
        }
      )
    ])
    .transform(val => {
...
export const emailField = (customError?: EValidationErrors) => {
  return z
    .string()
    .email({ message: customError ?? EValidationErrors.ERROR_INVALID_EMAIL })
}
...
export const onlyNumbersLettersCommasPointsField = (
  customError?: EValidationErrors
) => {
  ...
  return z.string().regex(onlyNumbersLettersCommasPointsRegex, {
    message:
      customError ??
      EValidationErrors.ERROR_MUST_CONTAIN_ONLY_NUMBERS_LETTERS_COMMAS_POINTS
  })
}

export const onlyNumbersLettersHyphenField = (
  customError?: EValidationErrors
) => {
